  ...
  return (
    <nav className='w-full z-50 flex justify-between bg-gray-800 text-white p-4'>
        ...
        <ul className='hidden md:flex space-x-4'>
            ...
            <li onClick={()=> {navigate("/")}}><a>Apps</a></li>
        </ul>
        ...
        <div className={isOpen?'absolute w-48 flex bg-white border-1 z-50 border-gray-200 left-0 top-0':'hidden'}>
          <ul className='flex-col h-full py-4 w-full text-gray-600'>
            <li onClick={()=> {navigate("/")}} className='w-full p-4 hover:bg-gray-100'><a >Home</a></li>
            <li onClick={()=> {navigate("/")}} className='w-full p-4 hover:bg-gray-100'><a>Apps</a></li>
        </ul>
        </div>
    </nav>
  )
